...
function pargs(argumentsValue) {
	var s = pCommon(Array.prototype.slice.call(argumentsValue))
	try {
		console.log(s)
	} catch (e) {}
...
*/
function pp() {
	var result = pCommon(Array.prototype.slice.call(arguments), deepOpts)
	try {
		console.log(result)
	} catch (e) {}
...
function pps() {
	return pCommon(Array.prototype.slice.call(arguments), deepOpts)
}

// utility functions

// argArray: array of arguments to be printed
function pCommon(argArray, options) {
	var result = []

	// prepend and location
	var location = getLocation(3)
	if (prepend)
		if (location) result.push(prepend + ':' + location)
		else result.push(prepend)
	else if (location) result.push(location)

	// values
	argArray.forEach(function (value) {
		result.push(inspect.inspect(value, options || defOpts))
	})
