...
.v the value being printed, it is not undefined or null
.result: array of string: output buffer
.getPrintableValue: function: allows for converting values to printable text
...
.g.opts object: provided options for printing - described in inspect.js
*/
function printProperties(p) {
	...
	var printed = 0
	var didPrint
	p.g.objects.push(p.v) // save this value to the list of previously seen objects
	...
	if (p.g.opts.maxLevels == null || p.g.levelCount < p.g.opts.maxLevels) for (;;) {
		...
		if (!p.g.opts.nonEnum) propertyList = Object.keys(propObj = p.v)
			...
			if (!propObj) propObj = p.v
				...
				if (propObj && propObj.constructor)
